...
/**
 * Applys the event value of the `EventStream[α, (β → γ)]` to the
...
EventStream.prototype.concat = function (es) {
  ...
  return new EventStream(function (next) {
    //TODO fix this
    var _fork = fork.bind(self, function (a) {
      if (a.isEmpty) _fork = es.fork.bind(es, function (b) { next(b); });
      else next(a);
    });
    _fork();
  });
