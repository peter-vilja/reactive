...
EventStream.prototype.chain = function _chain(fn) {
  ...
  return new EventStream(function (next) {
    ...
    subscribe(function (a) {
      if (child) { child(); }
      child = fn(a.value).subscribe(next);
    });
  });
};
...
var throttle = function _throttle(wait, fn) {
  var timeout, args, context, timestamp, result;
  var previous = 0;
 ...
 *
 * @api public
 * @type throttle :: @EventStream[α, β] => (α → Void) → (Void → Void)
 */

EventStream.prototype.throttle = function _throttle(wait) {
  var subscribe = this.subscribe.bind(this);
  return new EventStream(function (next) {
    subscribe(throttle(wait, next));
  });
};
...
Behavior.prototype.chain = function _chain(fn) {
  ...
  this.subscribe(function (a) {
    if (child) { child(); }
    child = fn(a).subscribe(b.next.bind(b));
  });
  return b;
};
...
Behavior.prototype.ap = function _ap(behavior) {
  ...
  this.subscribe(function (fn) {
    f = fn;
    b.next(f(behavior.observable.event));
  });
  behavior.subscribe(function (c) {
    b.next(f(c));
  });
  return b;
};
 ...
 *
 * @api public
 * @type bufferWithCount :: @Behavior[α] => α → β → Behavior[γ]
 */

Behavior.prototype.bufferWithCount = function _bufferWithCount(count, skip) {
  var b = new Behavior();
  skip = skip || count;
  var n = 0;
  var len;
  var buffer = [[]];
  this.subscribe(function (a) {
    for (var i = 0, len = buffer.length; i < len; i++) buffer[i].push(a);
    var diff = ++n - count;
